...
export function activate (context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('front-i18n.autoRecognition', () => {
  ...
  context.subscriptions.push(vscode.languages.registerHoverProvider({scheme: 'file'}, {
    provideHover(document, position, token) {
        ...
        if (i18nWindows[key].hasPostion(position)) { 
          return hoverText(`[$(trash) 移除翻译位置](command:front-i18n.removePosition?[${JSON.stringify({
            key: key,
            pos: {
              line: position.line,
        ...
        return hoverText(`[$(add) 添加到国际化](command:front-i18n.addText?[${JSON.stringify({
            key: key,
  ...
  context.subscriptions.push(vscode.commands.registerCommand('front-i18n.removePosition', ({pos, key}) => {
    if (!i18nWindows[key] || !i18nWindows[key].webView?.panel) {
      return;
    }
  ...
  context.subscriptions.push(vscode.commands.registerCommand('front-i18n.addText', ({text, start, end, key}) => {
    if (!i18nWindows[key] || !i18nWindows[key].webView?.panel) {
      return;
    }
